...
- AI、计时房的真人超时托管、赢墩展示结束和跨轮推进都由服务端执行；客户端不会自行推进权威状态。
...
3. 创建时可分别选择 AI 自动补位和真人行动是否倒计时；至少两名在线真人进入后，房主可在大厅显式增加或删除机器人。所有真人准备后，可按创建房间时的选择自动补满剩余席位，或使用已经补满的显式阵容开始。
4. 服务端通过 `@wizzard/game-core` 校验每次叫墩、选王牌和出牌 Intent。
5. `trick-result` 在约 1.5 秒后推进；`round-score` 在约 8 秒后推进，也允许真人提前继续。
6. 当前小游戏新建房默认不设置在线真人的行动截止时间；关闭“不倒计时”后为 30 秒，连续两次超时后转为 AI 托管。旧客户端未传计时选项时仍保持 30 秒行为。无倒计时房的当前玩家断线时会启动一个不投影到界面的 30 秒重连宽限，及时恢复即取消，逾期才由服务端代行动，避免弃局卡死。恢复连接会轮换恢复令牌并收回真人控制。
...
- `FriendRoomController` 独立于 `cc`，负责输入规范化、入口绑定、准备、机器人数量、开始与大厅离房命令、互斥 pending、连接状态和错误；它不执行或复制比赛规则。
...
- `room.create.payload.turnTimerEnabled?: boolean` 控制在线真人行动截止时间。当前小游戏显式发送 `false` 作为新房默认值；旧客户端省略时服务端按 `true` 处理并保留 30 秒托管。该设置只取消在线真人在选王牌、叫墩和出牌阶段的 deadline，不取消断线重连宽限、AI、赢墩展示或跨轮推进延迟。
- 旧服务端会严格拒绝未知的创建字段，因此发布顺序必须是先部署接受 `turnTimerEnabled` 的云托管服务端，再上传会发送该字段的小游戏客户端。
- 大厅机器人管理使用幂等目标值命令 `room.set-ai-count { aiCount }`；仅房主、仅大厅、至少两名在线真人时可以增加，服务端始终执行容量与身份校验。
- `room.set-ai-count` 是 v1 中新增的严格客户端命令。发布时必须先部署支持该命令的云托管服务端，再上传会发送它的体验版；新体验版活动期间不得回滚到拒绝该命令的旧服务端。
...
1. A 创建 3 人快速局，保持默认“出牌不倒计时 · 开”，并复制 6 位纯数字房间码；B 输入另一个昵称并用该房间码加入。
2. A、B 都点击准备；A 保持 AI 补位开启并点击开始。
3. 两端应自动从大厅进入联网 `MatchSceneView`，看到相同公开状态但只看到自己的私有手牌和合法牌。
4. 轮流完成选择王牌、叫墩和出牌，确认真人行动不显示倒计时且不会被超时托管；另建一间关闭“不倒计时”的房，确认倒计时归零后客户端只等待并收到服务端托管结果。
5. 用浏览器网络面板短暂离线再恢复；应先显示重连状态，随后恢复同一玩家、同一私有手牌和最新版本。
6. 完成快速局后由房主请求重赛；两端应返回原房间大厅，房间码不变且不会额外创建 Socket。

新建房间首版从数字 `2–9` 生成 6 位房间码，保证服务端先部署期间旧体验版仍可加入；协议仅为短期迁移兼容旧安全字母码，不再生成带字母的新码。房间码是公开定位码而不是身份凭证，高熵邀请令牌和恢复令牌保持不变。

微信小游戏好友房分享使用 `session.established` 在建房时下发的高熵 `inviteToken`。客户端把它随同一房间会话保存，分享 query 只发送 `inviteToken`，并通过 `imageUrl` 引用包根目录的版本化 5:4 `share-card-v1.jpg`；好友冷启动或从后台回到小游戏时直接发送现有 `room.join`，随机生成昵称，不再要求输入房间码。服务端仍负责校验令牌、房间阶段和容量。已在其他联网房间中的客户端不会被分享链接静默切换。
...
1. A 发送 `{"v":1,"type":"room.create","requestId":"host-create","payload":{"avatarKey":"bamboo-cat","displayName":"房主","maxPlayers":3,"mode":"quick","turnTimerEnabled":false}}`，保存 `session.established` 中的房间码和恢复令牌。省略 `turnTimerEnabled` 可单独验证旧客户端的 30 秒兼容路径。
...
6. 默认无倒计时房应允许真人持续等待；另建 `turnTimerEnabled:true` 的房，在行动阶段等待截止时间，过期 Intent 应被拒绝，随后两端应收到服务端托管动作。两种房的一墩和一轮都应由服务端自动推进。
