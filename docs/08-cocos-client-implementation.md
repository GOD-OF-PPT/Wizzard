...
- `GamePreferences` / `PlatformServices`：独立保存合法牌提示与震动偏好，并在微信首页出现前一次性注册动态原生分享；同一模块负责首页分享、房间邀请卡片、版本化 5:4 分享图、冷启动参数和 `onShow` 热启动邀请，浏览器、微信与内存回退不泄漏到规则核心；
...
2. 创建或加入弹窗每次打开时生成一个可编辑的随机昵称；创建时选择头像、3–6 人桌、快速/经典局、AI 补位和是否倒计时，其中“出牌不倒计时”每次默认开启；加入时输入 6 位纯数字房间码；
3. 控制器先执行昵称 trim 与 1–16 字符校验，并将房间码 trim 后按 6 位纯数字格式校验；房间码输入框使用数字键盘；
4. 进入大厅后显示房间码、相对座位、房主、在线/准备状态；已入座 AI 显示头像与名字，最后一个空位可作为房主的“添加机器人”快捷入口；
5. 所有真人准备，房主决定是否 AI 补位并开始；默认房不设置真人选王牌、叫墩和出牌 deadline，关闭“不倒计时”后由服务端使用 30 秒托管；同类 pending 命令在 ack 或 request error 前不能重复发送；
...
微信 release 构建已经在 `game.json` 与 `settings.json` 中声明 `resources` 普通分包。最大的 8 张 PNG 经无损重编码后逐像素比较均为 `AE=0`，其中 7 张产生体积缩减，`teahouse-table` 输出字节数不变；最新正式产物主包为 2,211,407 B（2.1090 MiB）、资源分包为 28,514,601 B（27.1936 MiB）、总包为 30,726,008 B（29.3026 MiB），已满足 4 MiB 主包和 30 MiB 总包限制。构建脚本会在缺少分包或任一包体门槛超限时失败；微信开发者工具中的人工包体分析仍是发布验收步骤。
...
- 微信开发者工具导入与人工包体分析；
- 微信登录、`connectContainer` 双真机分享直入闭环和发布后云托管监控；首页通用分享与携带邀请令牌的房间原生分享已注册；
- 联网牌局主动离房和返回好友房入口；分享卡片的冷启动/`onShow` 自动加入以及好友房大厅二次确认离房返回首页已完成；
